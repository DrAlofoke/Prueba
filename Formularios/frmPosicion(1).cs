...
namespace PARCIALDOS
{
    public partial class frmPosicion : Form
    {
        ...
        private void GuardarLinea()
        {
            ...
            string stQuery = "INSERT INTO POSICIONES (IDPOSICION, NOMBREDEPOSICION, IDDEPARTAMENTO) VALUES ( @A0, @A1, @A2);";
        ...
        private void BorrarDatos(string numPos)
        {
            ...
            SqlCommand borrar = new SqlCommand("DELETE FROM POSICIONES WHERE IDPOSICION = @PV", cndel);
        ...
        private void LlenarDataGridView()
        {
            ...
            SqlCommand cmd = new SqlCommand("      SELECT T1.IDPOSICION, " +
                                            "             T1.NOMBREDEPOSICION, " +
                                            "             T1.IDDEPARTAMENTO, " +
                                            "             T2.NOMBREDEPARTAMENTO " +
                                            "        FROM POSICIONES    T1 " +
                                            "  LEFT  JOIN DEPARTAMENTOS T2 ON T1.IDDEPARTAMENTO = T2.IDDEPARTAMENTO " +
                                            "    ORDER BY T1.IDPOSICION ASC", cnread);
        ...
        private void BuscarDatos()
        {
            ...
            if (records.Read())
            {
                txtNombrePosicion.Text = Convert.ToString(records["NOMBREDEPOSICION"]);
                txtDepartamento.Text = Convert.ToString(records["IDDEPARTAMENTO"]);
                lblDepartamento.Text = Convert.ToString(records["NOMBREDEPARTAMENTO"]);
            }
