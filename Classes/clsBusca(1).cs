...
namespace PARCIALDOS
{
    public class cnn
    {
        ...
        public static string db = "Server=DESKTOP-TFVTCNK\\SQLEXPRESS;" +
                                  "DataBase=DBSC06;" +
    ...
    public class Busco
    {
        public static string Posiciones(string numPosicion)
        {
            ...
            SqlCommand cmnd = new SqlCommand("SELECT NOMBREDEPOSICION FROM POSICIONES WHERE IDPOSICION = @PV", cnxn);  // aqui el script de sql
            ...
            {
                return Convert.ToString(rsd["NOMBREDEPOSICION"]); 
        ...
        public static string Departamento(string numDepto)
        {
            ...
            SqlCommand cmnd = new SqlCommand("SELECT NOMBREDEPARTAMENTO FROM DEPARTAMENTOS WHERE IDDEPARTAMENTO = @PV", cnxn);  // aqui el script de sql
            cmnd.Parameters.AddWithValue("@PV", numDepto);   // se le envia un parametro al script de sql
            SqlDataReader rsd = cmnd.ExecuteReader();           // ejecuta el script de sql

            if (rsd.Read())                                      // aqui pregunta que si tiene data el objeto rsd
            {
                return Convert.ToString(rsd["NOMBREDEPARTAMENTO"]); // si contiene data la retornara al textbox y saldra del metodo
            }
